...
import 'payment_otp_verify_screen.dart';
...
class _PaymentPasswordScreenState extends State<PaymentPasswordScreen> {
  ...
  Future<void> _verifyOtp() async {
    ...
    try {
      ...
      final response = await http
          .post(
            Uri.parse('${ApiConfig.baseUrl}/orders/verify-password'),
            headers: {
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => VerifyPaymentScreen(
                ...
                amount: widget.amount, // Pass the orderId
              ),
            ),
          );
        }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Paymetn Authentication'),
